...
<script>
    ...
    "errors": [
        ...
        {
            "institution": [
                "Global attribute 'institution' has wrong value. Found value: test"
            ]
        },
        {
            "acronym": [
                "Global attribute 'acronym' has wrong value. Found value: test"
            ]
        },
        {
            "author": [
                "Global attribute 'author': Persons must be given as last_name, first_name[, email]"
            ]
        },
        {
            "time": [
        ...
        },
        {
            "coordinate_transform": [
                "Cannot check geographic coordinates because of error in 'crs' variable."
            ]
        }
    ],
    "warnings": [
    ...
    function build_obj(id, name, children){
      return {
        id: id,
        name: name,
        children: children
    ...
    function mapper(input, start_idx){
      let result = [];
      let id = start_idx;
      if(isObj(input)){
        ...
        input.forEach(
          function (elm) {
            ...
            let n = inner_res[1];
            if(isArray(objs)){
                objs.forEach(function(elm){result.push(elm)});
            }else{
                result.push(objs);
            }
            id = id + n;
  ...
  export default {
    name: "tree_example",
     data: () =>  ({
      items: [build_obj(0, 'file_name', mapper(er, 1)[0])]
    })
  }
</script>
